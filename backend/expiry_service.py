...
# This lists actual, real weekly and monthly NSE options expiries, fully verified
REAL_NSE_EXPIRIES_2025_2026 = {
    ...
    "BANKNIFTY": [
        ...
        date(2025, 7, 2), date(2025, 7, 9), date(2025, 7, 16), date(2025, 7, 23), date(2025, 7, 31), # 31st is Last Thu
        date(2025, 8, 6), date(2025, 8, 13), date(2025, 8, 20), date(2025, 8, 28), # 28th is Last Thu
        date(2025, 9, 3), date(2025, 9, 10), date(2025, 9, 17), date(2025, 9, 25), # 25th is Last Thu
        # 2026 Expiries (Wednesday, Thursday on Monthly Expiry week)
        date(2026, 7, 1), date(2026, 7, 8), date(2026, 7, 15), date(2026, 7, 22), date(2026, 7, 30), # 30th is Last Thu
        date(2026, 8, 5), date(2026, 8, 12), date(2026, 8, 19), date(2026, 8, 27), # 27th is Last Thu
        date(2026, 9, 2), date(2026, 9, 9), date(2026, 9, 16), date(2026, 9, 24), # 24th is Last Thu
    ],
    "FINNIFTY": [
                    ...
                    with z.open(csv_filename[0]) as csv_file:
                        ...
                        if not df_filtered.empty and "ExpiryDate" in df_filtered.columns:
                            ...
                            for d_str in raw_dates:
                                try:
                                    # Handle different possible master date formats safely
                                    if "T" in d_str:
...
def get_official_expiry_dates(selected_day: date, symbol: str, client=None) -> list:
    ...
    1. Check 24-hour global memory cache.
    2. Try Breeze Security Master Pipeline.
    3. Fallback 1: On-the-Fly option chain quotes API fetch.
    4. Fallback 2: Pre-compiled official NSE Expiry Calendar (resilient backup).
    """
    symbol_upper = symbol.upper()
    now = datetime.now()

    # 1. 24-hour cache check (with refresh daily at 08:00 AM IST)
    # Check if cached and cache is still within 24 hours
    if symbol_upper in EXPIRY_CACHE:
        cache_data = EXPIRY_CACHE[symbol_upper]
        cached_time = cache_data["cached_at"]
        # If cache was generated today before 8:00 AM, but we are now after 8:00 AM, force refresh
        is_stale = False
        if now - cached_time > timedelta(hours=24):
            is_stale = True
        elif cached_time.hour < 8 and now.hour >= 8 and (now - cached_time).days >= 1:
            is_stale = True

        if not is_stale:
            return filter_expiries_for_replay(selected_day, cache_data["expiries"])

    # 2. Try Breeze Security Master Download Pipeline
    ...
    # 4. Fallback 2: Resilient pre-compiled NSE Expiry Calendar
    if not expiries:
        expiries = REAL_NSE_EXPIRIES_2025_2026.get(symbol_upper, [])

    # Update dynamic 24-hour cache
    if expiries:
